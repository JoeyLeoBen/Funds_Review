...
WITH x_year_returns AS (
    ...
    WHERE 
        fees IS NOT NULL 
        AND date_full >= (
            SELECT CAST((DATE(MAX(date_full)) - interval '236 months')::DATE AS TEXT)
            FROM {{ ref('returns_with_fees_categories') }}
...
SELECT 
    ...
    fund_company_id, 
    '20HB' AS return_type, 
    (POWER(mul(net_returns), 0.05063291) - 1) AS net_compounded_returns, 
...
GROUP BY 
    ...
    fund_company_id
HAVING 
    (POWER(mul(net_returns), 0.05) - 1) IS NOT NULL 
    AND COUNT(*) >= 237
