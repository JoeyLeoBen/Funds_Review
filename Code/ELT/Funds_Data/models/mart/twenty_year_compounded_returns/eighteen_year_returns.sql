...
WITH x_year_returns AS (
    ...
    WHERE 
        fees IS NOT NULL 
        AND date_full >= (
            SELECT CAST((DATE(MAX(date_full)) - interval '215 months')::DATE AS TEXT)
            FROM {{ ref('returns_with_fees_categories') }}
...
SELECT 
    ...
    fund_company_id, 
    '18Y' AS return_type, 
    (POWER(mul(net_returns), 0.0555555555555556) - 1) AS net_compounded_returns, 
...
GROUP BY 
    ...
    fund_company_id
HAVING 
    (POWER(mul(net_returns), 0.0555555555555556) - 1) IS NOT NULL 
    AND COUNT(*) >= 216
