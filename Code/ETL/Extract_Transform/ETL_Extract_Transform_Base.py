...
############################################################################################################################
# Base class for Extract and Transform operations
class ExtractTransform:
    def __init__(self, main_file_search_path: str, store_export_path: str) -> None:
        """
        Initializes the ExtractTransform class with search and export paths.

        Args:
            main_file_search_path (str): Path to search for input files.
            store_export_path (str): Path to store exported files.
        """
        self.main_file_search_path = main_file_search_path
        self.store_export_path = store_export_path

    # Extract data
    ################################################################
    def _get_files(self, prefix: str, suffix: str) -> List[str]:
        """
        Retrieve files with specified prefix and suffix from the search path.
        ...
        Raises:
            FileNotFoundError: If no files are found with the specified prefix and suffix.
        ...
        for i in os.listdir(path):
            if os.path.isfile(os.path.join(path, i)) and prefix in i and suffix in i:
                files.append(os.path.join(path, i))  # Append full paths to the files
        if not files:
            raise FileNotFoundError(
                f"No files with prefix '{prefix}' and suffix '{suffix}' found in '{path}'"
            )
    ...
    # Store raw data
    ################################################################
    def store_data(self, label: str, status: str, prefix: str) -> None:
        """
        ...
        if len(files) > 0:
            try:
                date_dir = os.path.join(
                    self.store_export_path, datetime.datetime.now().strftime("%Y-%m-%d")
                )
